...
class Langbutton extends \yii\bootstrap\Widget
{
    ...
    public function run()
    {
        $lang=Language::find()->where(['active'=>1])->all();
        $defLang=Yii::$app->params['default_lang'];
        if(isset(Yii::$app->session['_worklang'])){
            $curLang=Language::findOne(Yii::$app->session['_worklang']);
        }else{
            $curLang=$defLang;
        }
//        $curLang=Language::findOne(Configuration::getValue('_DEFAULT_LANG_'));
        if(sizeof($lang)>1) {
            return $this->render('_langbutton',[
                'cur'=>$curLang,
                'lang'=>$lang,
                'defLang'=>$defLang
            ]);
